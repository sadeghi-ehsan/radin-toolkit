...
> extends Omit<Props<Option, IsMulti, Group>, OmittedReactSelectProps> {
...
interface ReactSelectCreatableProps<
  ...
  IsMulti extends boolean = false,
  Group extends GroupBase<Option> = GroupBase<Option>
> extends Omit<CreatableProps<Option, IsMulti, Group>, OmittedReactSelectProps> {
  /** The type attribute specifies the type of <select> element to display. */
  type?: "creatable";
}
...
interface ReactSelectAsyncProps<
  ...
  IsMulti extends boolean = false,
  Group extends GroupBase<Option> = GroupBase<Option>
> extends Omit<AsyncProps<Option, IsMulti, Group>, OmittedReactSelectProps> {
  /** The type attribute specifies the type of <select> element to display. */
  type?: "async";
}

interface ReactSelectAsyncCreatableProps<
  Option = unknown,
  IsMulti extends boolean = false,
  Group extends GroupBase<Option> = GroupBase<Option>
> extends Omit<AsyncCreatableProps<Option, IsMulti, Group>, OmittedReactSelectProps> {
  /** The type attribute specifies the type of <select> element to display. */
  type?: "asyncCreatable";
}

interface ReactSelectAsyncPaginateProps<
  Option = unknown,
  IsMulti extends boolean = false,
  Group extends GroupBase<Option> = GroupBase<Option>,
  Add = {}
> extends Omit<AsyncPaginateProps<Option, Group, Add, IsMulti>, OmittedReactSelectProps> {
  /** The type attribute specifies the type of <select> element to display. */
  type?: "asyncPaginate";
}
...
> =
  ...
  | ReactSelectAsyncCreatableProps<Option, IsMulti, Group>;
...
export type ISelectProps<
  ...
  Group extends GroupBase<Option> = GroupBase<Option>,
  Add = {}
> = Omit<IFormElWrapperProps, "onClear" | "onFieldsetClick"> &
  (ReactSelectTypes<Option, IsMulti, Group> | ReactSelectAsyncPaginateProps<Option, IsMulti, Group, Add>) & {
    name: string;

    onChange?: (value: any, name: string, option?) => void;
    ...
    clearedValue?: string | boolean | { [key: string]: any };
