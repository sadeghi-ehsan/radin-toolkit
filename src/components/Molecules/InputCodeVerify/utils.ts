 ...
 */
export const handlePaste = (e: ClipboardEvent<HTMLInputElement>, { setCombinedValue, inputsLength }) => {
  e.preventDefault();
 ...
 */
export const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>, { itemsRef }) => {
...
export const handleChange = (v, n, { itemsRef, setCombinedValue }) => {
  ...
  setCombinedValue(prev =>
    !v ? prev.substring(0, InputIndex) : prev.substring(0, InputIndex) + v + prev.substring(InputIndex + v.length)
  );
