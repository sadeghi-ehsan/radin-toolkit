 ...
 */
export const generateValidationObj = ({ returningParams, schema, validationsToIgnore, defaultRequiredErrorMessage }) =>
  schema.reduce((prev, field) => {
    ...
    if (!rule && "required" in props && props?.required) {
      rule = yup
        .string()
        .required(
          defaultRequiredErrorMessage?.replace("$", !empty(props?.label) && isString(props?.label) ? props?.label : "")
        );
    }
 ...
 * form default values.
 */
export const formDefaultValuesObj = ({ returningParams, schema, formDefaultValues }) =>
  schema.reduce((prev, field) => {
    const { props } = field;
 ...
 */
export function modifyValidationSchema(
  fieldName: any,
  isAdd: boolean,
  { formProps, validationsToIgnore, setValidationsToIgnore }
) {
  if (isAdd) {
