 ...
 * @returns boolean
 */
export const isValidEmail = (value) => !!(value && /^[\w%+.-]+@[\d.a-z-]+\.[a-z]{2,4}$/i.test(value));
/**
 * check if value is valid Iranian mobile number
 * @param value
 * @returns boolean
 */
export const isValidIranMobileNumber = (value) => !!(value && /^0?9\d{9}$/g.test(value));
/**
 ...
 */
export function isValidIranBankCardNumber(input) {
    const L = input.length;
    if (L < 16 || parseInt(input.substr(1, 10), 10) === 0 || parseInt(input.substr(10, 6), 10) === 0)
        return false;
    let s = 0;
    let k;
    let d;
...
export function iso7064Mod97_10(iban) {
    let remainder = iban;
    let block;
 ...
 */
export function isValidIranShebaNumber(str) {
 ...
 * @returns boolean
 */
export const isValidIpAddress = (value) => !!(value && /\b(?:(?:2(?:[0-4]\d|5[0-5])|[01]?\d?\d)\.){3}(?:2([0-4]\d|5[0-5])|[01]?\d?\d)\b/i.test(value));
/**
 ...
 */
export const isValidIranPostalCode = (value) => !!(value && /\b(?!(\d)\1{3})[13-9]{4}[1346-9][013-9]{5}\b/i.test(value));
