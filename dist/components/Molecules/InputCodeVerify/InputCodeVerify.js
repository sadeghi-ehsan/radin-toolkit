...
export const InputCodeVerify = forwardRef(({ className, size, id: _id, onChange, disabled, readOnly, value, name, style, inputsLength, placeholder, onResend, counterSeconds, autoFocusFirstField, isLoading, resendBtnText, ...restProps }, passedRef) => {
    ...
    return (_jsx(FormElementWrapper, { ...wrapperProps, ...{
            ...
            endLabelAdornment: (_jsx(RenderEndLabelAdornment, { ...{ isLoading, disabled, onResend, handleResend, counter, resendBtnText } }))
        }, variant: "none", childrenClassName: "flex flex-nowrap items-center dir-ltr justify-center h-auto py-0 pl-0", children: _jsx("div", { children: [...Array(inputsLength).keys()].map(i => (_jsx("div", { className: classJoin(["flex-1 text-center h-auto px-1 first:pl-0 last:pr-0"]), children: _jsx(BaseInput, { name: `part${i}`, id: i === 0 ? id : undefined, ref: el => {
                        itemsRef.current[i] = el;
                    }, autoFocus: autoFocusFirstField && i === 0, autoComplete: "off", onFocus: () => setFocused(true), onBlur: () => setFocused(false), value: combinedValue?.charAt(i), type: "number", align: "center", onChange: (e, n = name) => handleChange(e, n, { itemsRef, setCombinedValue }), disabled: disabled || isLoading, readOnly: readOnly, fontSize: size === "sm" ? "xs" : "sm", onKeyDown: e => handleKeyDown(e, { itemsRef }), onPaste: e => handlePaste(e, { setCombinedValue, inputsLength }), className: classJoin([
                        ...
                        ifNotEmpty(wrapperProps.error, "border-error shadow-input shadow-error", focused || value ? "border-primary" : "border-disabled")
                    ]), placeholder: placeholder }) }, i))) }) }));
});
InputCodeVerify.defaultProps = {
    ...filterObj(Input.defaultProps, (v, k) => ![
        "variant",
        "type",
        "align",
        "onPaste",
...
//# sourceMappingURL=InputCodeVerify.js.map