...
// Select props
className, clearedValue, searchable, id: _id, onChange, onInputChange, disabled, value, name, style, loading, multi, placeholder, type, onMenuOpen, onMenuClose, 
// FormElementWrapper props,
size, clearable, childrenClassName, fieldsetClassName, customComponents, ...restProps }, passedRef) => {
    ...
    const handleInputChange = v => {
        if (!empty(onInputChange) && isCallable(onInputChange) && v)
            onInputChange(v);
    };
    const onClear = () => {
    ...
    return (_jsx(FormElementWrapper, { ...wrapperProps, ...{
            ...
            fieldsetClassName: classJoin([
                ...
                ifNotEmpty(open, "rounded-b-none border-b-transparent bg-red hover:border-b-transparent focus-within:border-b-transparent"),
                fieldsetClassName
            ])
        }, children: _jsx(SelectComponent, { ...componentProps, ...{ [type === "asyncPaginate" ? "selectRef" : "ref"]: ref }, classNamePrefix: "select", value: value, id: id, name: name, placeholder: placeholder, onChange: handleChange, onInputChange: handleInputChange, menuPlacement: "auto", menuPosition: "absolute", menuPortalTarget: document.body, styles: customStyles(), theme: customTheme(), components: {
            ...
            // bordered={false}
            isDisabled: disabled, isSearchable: searchable || ["creatable", "async", "asyncCreatable"].includes(type), isLoading: loading, isClearable: clearable, isMulti: multi, isRtl: true, parentRef: ref }) }));
});
Select.defaultProps = {
