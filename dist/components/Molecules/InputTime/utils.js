...
export const InputTimeGroup = ({ inputGroupIndex, groupLabel, inputVariant, min, max, disabled, error, required, value, onChange, onPaste, focusField }) => {
     ...
     */
    const parseValue = (ones, tens) => Number(`${tens}${ones}`);
    /**
     ...
     */
    const onKeyUp = (targetIndex) => (e) => {
        e.preventDefault();
        if (/^\d$/i.test(e.key))
            focusField(targetIndex + 1);
    };
    return (_jsx(FormElementWrapper, { label: groupLabel, fieldsetClassName: classJoin(["flex flex-row justify-center !h-14 !w-17.5 dir-ltr"]), childrenClassName: "w-full", legendClassName: "!mx-auto", variant: inputVariant, disabled: disabled, error: error, required: required, children: _jsxs("div", { className: "flex flex-row space-x-1 justify-center", children: [_jsx(InputNumber, { min: 0, max: 9, className: "!p-0", onPaste: onPaste, name: `${groupLabel}-0`, variant: "line", counterType: "none", disabled: disabled, fieldsetClassName: "!ml-0 !h-auto", legendClassName: "!mx-auto", baseInputClassName: "!text-center !p-0", onKeyDown: onKeyDown(2 * inputGroupIndex), onClick: onClick(2 * inputGroupIndex), onKeyUp: onKeyUp(2 * inputGroupIndex), value: tensValue, onChange: v => {
                        const newValue = parseValue(Number(onesValue) || 0, Number(v) || 0);
                        updateValue(newValue);
                    } }), _jsx(InputNumber, { min: 0, max: 9, className: "!p-0", onPaste: onPaste, name: `${groupLabel}-1`, variant: "line", counterType: "none", disabled: disabled, fieldsetClassName: "!m-0 !h-auto", legendClassName: "!mx-auto", baseInputClassName: "!text-center !p-0", onKeyDown: onKeyDown(2 * inputGroupIndex + 1), onClick: onClick(2 * inputGroupIndex + 1), onKeyUp: onKeyUp(2 * inputGroupIndex + 1), value: onesValue, onChange: v => {
